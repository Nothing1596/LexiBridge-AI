...
- Legacy `/api/alignment/run` remains an active compatibility surface after Task 9C.5L's consumer audit. No production frontend POST consumer remains, but compatibility tests, readiness probes, OpenAPI contract, unresolved runtime jobs, and unknown external clients prevent removal or HTTP 410 in this candidate. Task 9C.5H removes it from the teacher document-alignment start, polling, item-loading, and reload-recovery paths; Task 9C.5N.1 gives queued legacy work a dedicated worker mode and a default-enabled route admission flag without changing the compatibility response while enabled. Its external/live execution path remains blocked.
