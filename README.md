...
- Task 9C.4V defines the replacement contract as `FORMAL_DOCUMENT_ALIGNMENT_ORCHESTRATION` with governed `KnowledgeSource` input, async workflow execution, formal provider policy/preflight/verification, `Idempotency-Key`, no legacy/formal dual write, and future `POST /api/document-alignment-runs` / status / items APIs. Task 9C.4W adds the required `DocumentAlignmentWorkflowRun` and `DocumentAlignmentWorkflowItem` models for SQLite/create_all small-pilot use only. The application service, routes, worker orchestration, frontend cutover, OpenAPI contract, production migration framework, and PostgreSQL validation are not implemented yet.
