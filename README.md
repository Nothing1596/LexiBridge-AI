...
- Task 9C.4V defines the replacement contract as `FORMAL_DOCUMENT_ALIGNMENT_ORCHESTRATION` with governed `KnowledgeSource` input, async workflow execution, formal provider policy/preflight/verification, `Idempotency-Key`, no legacy/formal dual write, and future `POST /api/document-alignment-runs` / status / items APIs. Task 9C.4W adds the required root/item models for SQLite/create_all small-pilot use only, and Task 9C.4X adds only admission/start. Task 9C.4Y characterizes processing. Task 9C.4Z establishes formal BackgroundJob CAS claim/reclaim, 30-second heartbeat lease, execution-attempt/token fencing, terminal immutability, and legacy-worker isolation for the local pilot: `FORMAL_BACKGROUND_JOB_EXECUTION_OWNERSHIP_ESTABLISHED_FOR_LOCAL_PILOT`, `AT_LEAST_ONCE_TRANSPORT`, and `ATTEMPT_FENCED_OWNERSHIP`. It does not provide exactly-once processing. `PILOT_CREATE_ALL_ONLY`, `FORMAL_MIGRATION_REQUIRED_BEFORE_PRODUCTION`, `POSTGRESQL_LEASE_SEMANTICS_NOT_VERIFIED`, the missing formal worker/processing/routes/frontend, and the next `SPLIT_TERM_EXTRACTION_AND_ITEM_PERSISTENCE_FIRST` blocker remain explicit.
