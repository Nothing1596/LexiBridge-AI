...
The local loop alternates first opportunity between the formal document
alignment CAS dispatcher and the legacy queue, processing at most one job per
iteration. This is a controlled-pilot worker, not a supervised production
...
recovery, and same-origin browser API access on local SQLite. Frontend
polling/cutover, legacy disablement, PostgreSQL, a supervised worker, and
live-provider operation remain future work.
