...
class LayoutProviderUnavailable(RuntimeError):
    pass
...
def parse_pdf_layout(path, provider=None):
    requested_provider = normalize_provider_name(
        provider or os.environ.get("LAYOUT_PROVIDER", PROVIDER_RULE_BASED)
    )

    if requested_provider == PROVIDER_DOCLAYOUT_YOLO_ONNX:
        try:
            return _parse_pdf_with_doclayout_yolo_onnx(path)
        except LayoutProviderUnavailable as exc:
            fallback = _parse_pdf_with_rule_based_layout(path)
            return _with_warning(fallback, f"onnx_provider_unavailable:{exc}")

    if requested_provider != PROVIDER_RULE_BASED:
        fallback = _parse_pdf_with_rule_based_layout(path)
        return _with_warning(fallback, f"unknown_layout_provider:{requested_provider}")

    return _parse_pdf_with_rule_based_layout(path)
...
def _parse_pdf_with_rule_based_layout(path):
    """
    Extract a lightweight layout model from embedded PDF text.

    This rule-based provider is intentionally small: it gives the application a
    stable data contract and a deterministic fallback before heavier OCR/layout
    model providers are introduced.
    """
    try:
        import fitz
...
def extract_pdf_text_with_layout(path):
    text, _ = extract_pdf_text_with_layout_result(path)

    return text


def extract_pdf_text_with_layout_result(path):
    result = parse_pdf_layout(path)

    if result.needs_ocr_engine:
        return "", result

    return layout_blocks_to_text(result.blocks), result
...
def _parse_pdf_with_doclayout_yolo_onnx(path):
    model_path = os.environ.get("LAYOUT_MODEL_PATH", "").strip()

    if not model_path:
        raise LayoutProviderUnavailable("LAYOUT_MODEL_PATH is not set")

    if not os.path.exists(model_path):
        raise LayoutProviderUnavailable("LAYOUT_MODEL_PATH does not exist")
    ...
    try:
        ...
        raise LayoutProviderUnavailable(str(exc)) from exc

    try:
        session = _get_onnx_session(ort, model_path)
    except Exception as exc:
        raise LayoutProviderUnavailable(f"failed to load onnx model: {exc}") from exc
    ...
    with fitz.open(path) as doc:
        for page_index, page in enumerate(doc, start=1):
            try:
                blocks.extend(_detect_model_blocks(page, page_index, session, np, fitz))
            except Exception as exc:
...
def _get_onnx_session(ort, model_path):
...
def _detect_model_blocks(page, page_number, session, np, fitz):
    ...
    detections = _normalize_model_output(outputs[0], np)
    score_threshold = _model_score_threshold()
...
def _normalize_model_output(output, np):
    ...
    if data.shape[0] == raw_attribute_count:
        return _decode_raw_yolo_output(data.T, np)

    if data.shape[-1] == raw_attribute_count:
        return _decode_raw_yolo_output(data, np)

    raise UnsupportedModelOutput(f"unsupported output shape: {data.shape}")


def _decode_raw_yolo_output(predictions, np):
    """
    ...
    keep = scores >= _model_score_threshold()
    ...
    keep_indices = _nms(boxes_xyxy, scores, np)
...
def _nms(boxes, scores, np):
    ...
    keep = []
    threshold = _model_nms_iou_threshold()
...
def _model_score_threshold():
    return _float_env("LAYOUT_MODEL_SCORE_THRESHOLD", DEFAULT_MODEL_SCORE_THRESHOLD)


def _model_nms_iou_threshold():
    return _float_env("LAYOUT_MODEL_NMS_IOU_THRESHOLD", DEFAULT_MODEL_NMS_IOU_THRESHOLD)
