...
RUNNER = ROOT / "scripts/evaluations/chinese_term_identification_v2.py"
...
def _runner():
    spec = importlib.util.spec_from_file_location("chinese_term_identification_v2", RUNNER)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_v2_runner_preserves_denominators_and_upstream_failures(app_module):
    del app_module
    ...
    assert metrics["english_missing"] == 3
    assert metrics["english_ambiguous"] == 4
    assert sum(
