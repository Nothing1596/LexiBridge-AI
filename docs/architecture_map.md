...
- This is a staged extraction, not a complete `backend/app.py` modularization. Upload/knowledge/evidence, course management, policy management, provider execution/replay follow-up routes, admin views, and other remaining domains are still registered directly in `backend/app.py`.
