...
- This is a staged extraction, not a complete `backend/app.py` modularization. Upload/knowledge/evidence, course management, policy management, legacy `/api/alignment/run` and `/api/alignment/runs*`, legacy provider admin views, provider healthcheck, admin views, and other remaining domains are still registered directly in `backend/app.py`.
