...
- Task 9C.4Q still does not extract `POST /api/admin/ai/prompts`. It establishes `backend/services/legacy_provider_prompt_mutation.py`, which implements `LEGACY_PROMPT_MUTABLE_REVISION_V1`, owns seed integration, key/version lookup, mutable revision create/update, one commit, and explicit rollback. The app callback is now a thin HTTP adapter that maps the typed service result to the existing legacy envelope.
...
- This is a staged extraction, not a complete `backend/app.py` modularization. Upload/knowledge/evidence, course management, policy management, legacy `/api/alignment/run` and `/api/alignment/runs*`, prompt mutation route wiring, admin views, and other remaining domains are still registered directly in `backend/app.py`. Prompt mutation service extraction is complete; the next prompt slice should move only the thin POST HTTP adapter while preserving the shared `admin_ai_prompts` contract.
