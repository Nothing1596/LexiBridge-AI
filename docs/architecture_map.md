...
- Task 9C.4O does not extract or modify `POST /api/admin/ai/prompts`. It characterizes the app-owned prompt mutation callback in `docs/legacy_provider_prompt_mutation_boundary.md` and concludes `PROMPT_VERSIONING_OR_CONCURRENCY_POLICY_REQUIRED_FIRST`: the current POST route is an upsert keyed by `prompt_key` + `prompt_version`, commits directly, has no explicit rollback, and does not define uniqueness, optimistic locking, or active/default exclusivity policy.
...
- This is a staged extraction, not a complete `backend/app.py` modularization. Upload/knowledge/evidence, course management, policy management, legacy `/api/alignment/run` and `/api/alignment/runs*`, prompt mutation, admin views, and other remaining domains are still registered directly in `backend/app.py`. Prompt mutation should next move into an application service that implements the accepted 9C.4P compatibility policy before the POST route itself is extracted.
