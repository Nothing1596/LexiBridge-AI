...
- The legacy provider admin configuration route module also uses explicit `app.add_url_rule` registration. It performs admin authentication, calls the seed service directly, reads `AIProviderConfig`, `AIModelRegistry`, and `PromptTemplate`, and returns existing serializers passed explicitly from `backend/app.py`. It does not own commit/rollback, registry mutation beyond the existing seed flush, healthcheck execution, provider transport, credential access, or prompt mutation logic.
- `POST /api/admin/ai/healthcheck` remains app-owned. It still owns provider health commits and possible seed persistence. The local readiness portion should become one service, while any live transport probe should become a separate service or be disabled/deprecated first.
...
- This is a staged extraction, not a complete `backend/app.py` modularization. Upload/knowledge/evidence, course management, policy management, legacy `/api/alignment/run` and `/api/alignment/runs*`, provider healthcheck, prompt mutation, admin views, and other remaining domains are still registered directly in `backend/app.py`.
