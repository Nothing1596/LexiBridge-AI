...
- This is a staged extraction, not a complete `backend/app.py` modularization. Provider preflight execution, alignment verification execution, upload/knowledge/evidence, course management, policy management, and other remaining domains are still registered directly in `backend/app.py`.
