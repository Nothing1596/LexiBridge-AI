...
- `POST /api/admin/ai/healthcheck` remains app-owned. It still owns provider health commits and possible seed persistence, while `services/legacy_provider_local_readiness.py` owns local readiness decisions and the `LEGACY_LIVE_PROBE_DISABLED` result. Legacy live transport probing is disabled and must not be reopened without a separate explicit service.
