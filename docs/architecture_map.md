...
- `POST /api/admin/ai/healthcheck` remains app-owned. It still owns provider health commits and possible seed persistence. The local readiness portion should become one service. Legacy live transport probing is disabled and must not be reopened without a separate explicit service.
