...
- Alignment verification: `services/alignment_verification.py`, `services/alignment_providers.py`, `services/alignment_prompting.py`, and `services/alignment_output_parser.py` define provider contracts, prompt/schema versions, mock/fake/replay behavior, and parser failure handling.
...
- `POST /api/alignment/verify` still lives in `backend/app.py`. Its current handler directly coordinates request parsing, card lookup, provider governance, provider execution, `AlignmentVerificationRun`, `AlignmentProviderUsageRecord`, optional card attach, audit sequencing, and transaction rollback. Route extraction should wait until that orchestration is behind an explicit alignment-verification execution service boundary.
...
- This is a staged extraction, not a complete `backend/app.py` modularization. Alignment verification execution, upload/knowledge/evidence, course management, policy management, and other remaining domains are still registered directly in `backend/app.py`.
