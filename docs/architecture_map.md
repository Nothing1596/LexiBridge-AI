...
- This is a staged extraction, not a complete `backend/app.py` modularization. Provider execution/mutation, upload/knowledge/evidence, course management, policy management, and other remaining domains are still registered directly in `backend/app.py`.
