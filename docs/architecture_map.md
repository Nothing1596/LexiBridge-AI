...
- `POST /api/alignment/verify` still lives in `backend/app.py`. Its current handler is a thin adapter for auth, request parsing, request ID/audit context, DTO construction, service invocation, and response mapping. `services/alignment_verification_execution.py` owns provider selection, provider governance, `AlignmentVerificationRun`, `AlignmentProviderUsageRecord`, mock/fake/replay/disabled execution dispatch through existing services, optional card attach, audit sequencing, and business transaction rollback. The next extraction should move only this adapter into a route module.
...
- This is a staged extraction, not a complete `backend/app.py` modularization. Alignment verification route registration, upload/knowledge/evidence, course management, policy management, and other remaining domains are still registered directly in `backend/app.py`.
